...
namespace lxh {
	...
	{
		// sort lights
		std::map<float, LxhGameObject::id_t> sorted;
		for (auto& kv : frameInfo.gameObjects) {
			auto& obj = kv.second;
			if (obj.pointLight == nullptr) continue;
	...
	void PointLightSystem::createPipelineLayout(VkDescriptorSetLayout globalSetLayout)
	{
		...
		if (vkCreatePipelineLayout(lxhDevice.getDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
		{
	...
	void PointLightSystem::createPipeline(VkRenderPass renderPass)
	{
		...
		LxhPipeline::enableAlphaBlending(pipelineConfig);
